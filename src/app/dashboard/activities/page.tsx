...
import { Plus, Check, Clock, Phone, Mail, Video, FileText } from "lucide-react";
import { formatDate } from "@/lib/utils";

interface Activity {
  id: string; type: string; title: string; description: string | null; dueDate: string | null; completed: boolean;
...
export default function ActivitiesPage() {
  ...
  const fetchActivities = async () => {
    setLoading(true);
    try { const res = await fetch("/api/activities"); const data = await res.json(); setActivities(data.activities || []); }
    catch (error) { console.error(error); } finally { setLoading(false); }
  };

  const handleComplete = async (id: string) => {
    await fetch("/api/activities", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, completed: true }) });
    fetchActivities();
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch("/api/activities", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: formData.type, title: formData.title, description: formData.description, dueDate: formData.dueDate || undefined }) });
    if (res.ok) { setDialogOpen(false); setFormData({ type: "CALL", title: "", description: "", dueDate: "" }); fetchActivities(); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold text-gray-900">Atividades</h2><p className="text-gray-600">Gerencie suas tarefas e atividades</p></div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2"><Label>Data/Hora</Label><Input type="datetime-local" value={formData.dueDate} onChange={(e) => setFormData({ ...formData, dueDate: e.target.value })} /></div>
      ...
      {loading ? <div className="text-center py-12 text-gray-500">Carregando...</div> : activities.length === 0 ? (
        <div className="text-center py-12 text-gray-500">Nenhuma atividade encontrada</div>
      ) : (
        <div className="space-y-4">
          {pending.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-lg">Pendentes</CardTitle></CardHeader>
                  ...
                  return (
                    <div key={activity.id} className="flex items-center justify-between rounded-lg border p-4 hover:bg-gray-50">
                      <div className="flex items-center space-x-4">
                        <div className="rounded-lg bg-gray-100 p-2"><Icon className={`h-5 w-5 ${info.color}`} /></div>
                        <div>
                      ...
                      <Button variant="ghost" size="icon" onClick={() => handleComplete(activity.id)}><Check className="h-4 w-4" /></Button>
                    </div>
                  );
                })}
              </CardContent>
          ...
          {completed.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-lg text-gray-500">Concluídas</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {completed.map((activity) => {
                  ...
                  return (
                    <div key={activity.id} className="flex items-center justify-between rounded-lg border p-4 bg-gray-50 opacity-60">
                      <div className="flex items-center space-x-4">
                        <div className="rounded-lg bg-gray-200 p-2"><Icon className="h-5 w-5 text-gray-400" /></div>
